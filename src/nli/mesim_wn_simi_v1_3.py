# This file created on Aug 30, 2018.
...
def train_fever_v1_advsample():
    ...
    train_sentence_shuffle = True
    start_eval_epoch = 12
    experiment_name = f"emc_mesim_wn_doc_simi_v12_{train_sentence_shuffle}|t_prob:{train_prob_threshold}|top_k:{train_sample_top_k}"
    ...
    print("Building Prob Dicts fod docs...")
    train_doc_list = common.load_jsonl(
        config.PRO_ROOT / "saved_models/08-26-15:46:10_simple_nn_doc_first_sent/extra_needed_training_data/train_doc_list.jsonl")
    # remaining_doc_list = common.load_jsonl(
    #     config.RESULT_PATH / "sent_retri_nn/remaining_training_cache/remain_train_sent.jsonl")
    dev_doc_list = common.load_jsonl(config.PRO_ROOT / "saved_models/08-26-15:46:10_simple_nn_doc_first_sent/extra_needed_training_data/dev_doc_list.jsonl")

    selection_dict_with_doc = paired_selection_score_dict_for_doc(train_doc_list)
    selection_dict_with_doc = paired_selection_score_dict_for_doc(dev_doc_list, selection_dict_with_doc)
    # selection_dict_with_doc = paired_selection_score_dict(remaining_sent_list, selection_dict_with_doc)
    ...
    dev_fever_data_reader = WNDocSIMIReader(token_indexers=token_indexers, lazy=lazy, wn_p_dict=p_dict, max_l=360)
    train_fever_data_reader = WNDocSIMIReader(token_indexers=token_indexers, lazy=lazy, wn_p_dict=p_dict, max_l=360,
                                              shuffle_sentences=train_sentence_shuffle)

    complete_upstream_dev_data = select_sent_with_prob_doc_for_eval(config.T_FEVER_DEV_JSONL, upstream_dev_list,
                                                                    selection_dict,
                                                                    selection_dict_with_doc,
                                                                    tokenized=True)
    ...
    for i_epoch in range(num_epoch):
        ...
        complete_upstream_train_data = adv_simi_sample_with_prob_v1_0_with_doc(config.T_FEVER_TRAIN_JSONL,
                                                                               train_data_with_candidate_sample_list,
                                                                               selection_dict,
                                                                               selection_dict_with_doc,
                                                                               tokenized=True)

        for item in complete_upstream_train_data:
            for t in item['evid']:
                if len(t) != 3:
                    print(item)
        ...
        for i, batch in tqdm(enumerate(train_iter)):
            ...
            if iteration % mod == 0:
                ...
                if strict_score > ema_best_dev:
                    ema_best_dev = strict_score
...
def train_fever_v1_advsample_shuffle_bigger():
    ...
    # print("Building Prob Dicts fod docs...")
    train_doc_list = common.load_jsonl(
        config.RESULT_PATH / "sent_retri_nn/2018_07_20_15:17:59_r/train_doc_list.jsonl")
    # remaining_doc_list = common.load_jsonl(
    #     config.RESULT_PATH / "sent_retri_nn/remaining_training_cache/remain_train_sent.jsonl")
    dev_doc_list = common.load_jsonl(config.RESULT_PATH / "sent_retri_nn/2018_07_20_15:17:59_r/dev_doc_list.jsonl")
...
def pipeline_nli_run(t_org_file, upstream_dev_data_list,
                     upstream_sent_file_list,
                     upstream_doc_file_list,
                     model_path,
    ...
    print("Building Doc Prob Dicts...")
    selection_dict_with_doc = dict()
    for upstream_doc_file in upstream_doc_file_list:
        upstream_doc_l = common.load_jsonl(upstream_doc_file)
        selection_dict_with_doc = paired_selection_score_dict_for_doc(upstream_doc_l, selection_dict_with_doc)
    ...
    dev_fever_data_reader = WNDocSIMIReader(token_indexers=token_indexers, lazy=lazy, wn_p_dict=p_dict, max_l=420)

    complete_upstream_dev_data = select_sent_with_prob_doc_for_eval(t_org_file,
                                                                    upstream_dev_data_list,
                                                                    selection_dict,
                                                                    selection_dict_with_doc,
                                                                    tokenized=True, pipeline=True)
...
if __name__ == "__main__":
    ...
    # r_model = str(
    #     config.PRO_ROOT / "saved_models/07-22-14:41:21_mesim_wn_simi_v12_|t_prob:0.35|top_k:10/i(77000)_epoch(11)_dev(0.6601160116011601)_loss(1.1138329989302813)_seed(12)")

...
def train_fever_std_ema_v1(resume_model=None, do_analysis=False):
    ...
    schedule_sample_dict = defaultdict(lambda: 0.08)
    ...
    schedule_sample_dict.update({
        ...
        2: 0.08, 3: 0.08,   # 200k + 200k ~ 200k + 100k
        4: 0.08, 5: 0.08,     # 200k + 100k
        6: 0.08            # 20k + 20k
    })
    ...
    experiment_name = f"vc_ss_v18_ratio_ss_for_vc:{ratio_ss_for_vc}|t_prob:{train_prob_threshold}|top_k:{train_sample_top_k}_scheduled_neg_sampler"
    # resume_model = None
    ...
    for i_epoch in range(num_epoch):
        ...
        if i_epoch >= 1:    # if epoch num >= 6 we balance pos and neg example for selection
            ...
            cur_train_ss_data = pos_ss_data[:int(ss_sample_size * pos_sample_ratio)] + neg_ss_data[:ss_sample_size]

...
from sentence_retrieval.sampler_for_nmodel import get_full_list, get_additional_list
...
def compute_mixing_loss(model, out, batch, criterion, data_sampler: vc_ss.data_wrangler.VCSSTrainingSampler=None,
    ...
    for i in range(batch_size):
        ...
        if c_scorer.SENT_LINE in pid: # SS
            ...
            # We also use ss to train vc here
            if ss_for_vc_prob > 0 and batch['label'][i] == non_selection_index:    # if it is false, then we use it as NEI.
                # filp a coin here
                fcoin_prob = random.uniform(0, 1)
                if fcoin_prob < ss_for_vc_prob:
                    vc_batch_list.append(out[i][:3])
                    vc_y_list.append(batch['label'][i].new_full(batch['label'][i].size(), nei_index)) # 2 is NEI
...
def eval_sent_for_sampler(model, token_indexers, vocab, vc_ss_training_sampler, nei_only=False):
    max_len = 80
    batch_size = 64
...
def train_fever_std_ema_v1(resume_model=None, do_analysis=False):
    ...
    schedule_sample_dict = defaultdict(lambda: 0.005)

    ratio_ss_for_vc = 0.2

    schedule_sample_dict.update({
        0: 0.1, 1: 0.1,     # 200k + 400K
        2: 0.05, 3: 0.05,   # 200k + 200k ~ 200k + 100k
        4: 0.025, 5: 0.025, # 200k + 100k
        6: 0.005            # 20k + 20k
    })

    # Eval at beginning of the training.
    eval_full_epoch = 1
    eval_nei_epoches = [2, 3, 4]

    neg_only = False
    debug = False

    experiment_name = f"vc_ss_v15_ratio_ss_for_vc:{ratio_ss_for_vc}|t_prob:{train_prob_threshold}|top_k:{train_sample_top_k}_scheduled_neg_sampler"
    # resume_model = None
    ...
    for i_epoch in range(num_epoch):
        ...
        if i_epoch == eval_full_epoch:  # only eval at 1
            ...
            print("Be patient and hope good luck!")
            load_ema_to_model(cloned_empty_model, ema)
            eval_sent_for_sampler(cloned_empty_model, token_indexers, vocab, vc_ss_training_sampler, nei_only=True)
        ...
        if i_epoch >= 6:    # if epoch num >= 6 we balance pos and neg example for selection
            # new_ss_data = []
            pos_ss_data = []
            neg_ss_data = []
            ...
            cur_train_ss_data = pos_ss_data[:ss_sample_size] + neg_ss_data[:ss_sample_size]
...
def cv_ss_do_eval_ss(model_path, tokenized_file, dev_doc_upstream_file):
    batch_size = 32
    lazy = True
    train_prob_threshold = 0.02
    train_sample_top_k = 8
    ...
    complete_upstream_dev_data = get_full_list(tokenized_file, dev_doc_upstream_file, pred=True,
                                               top_k=top_k_doc)
    ...
    return scored_dev_sent_data


def cv_ss_do_eval_ss_multihop(model_path, tokenized_file, additional_file,
    ...
    additional_sentence_list = get_additional_list(tokenized_file, additional_file, item_key=item_key, top_k=top_k)

    print("Dev size:", len(additional_sentence_list))
    ...
    vc_ss.data_wrangler.assign_task_label(additional_sentence_list, 'ss')
    dev_ss_instance = dev_fever_data_reader.read(additional_sentence_list)
    eval_ss_iter = biterator(dev_ss_instance, num_epochs=1, shuffle=False)
    additional_sentence_sent_data = hidden_eval_ss(cloned_empty_model, eval_ss_iter, additional_sentence_list)

    return additional_sentence_sent_data


def cv_ss_do_eval_cv(t_org_file, upstream_dev_data_list, upstream_sent_file_list, model_path):
    batch_size = 32
    lazy = True
    train_prob_threshold = 0.02
    train_sample_top_k = 8
    ...
    # for item in upstream_dev_data_list:
    #     item['label'] = 'hidden'

    ready_dev_list = select_sent_with_prob_for_eval(t_org_file, upstream_dev_data_list,
                                                    selection_dict, tokenized=True, pipeline=True)
    for item in ready_dev_list:
        item['label'] = 'hidden'
...
if __name__ == "__main__":
    ...
    analysis_model("/home/easonnie/projects/FunEver/saved_models/12-05-00:24:50_vc_ss_v15_ratio_ss_for_vc:0.2|t_prob:0.02|top_k:8_scheduled_neg_sampler/ema_i(118000)_epoch(7)_dev(0.6462646264626463)_lacc(0.6867186718671867)_p(0.3392139213920993)_r(0.8655115511551155)_f1(0.4874032698200112)_seed(12)")
